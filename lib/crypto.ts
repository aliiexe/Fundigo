...
function getKey(): Buffer {
  ...
  if (!raw || raw.length < 32)
    throw new Error("MASTER_ENC_KEY is missing or invalid. Generate: openssl rand -base64 32");
  const buf = Buffer.from(raw, "base64");
  if (buf.length !== KEY_LENGTH) throw new Error(`MASTER_ENC_KEY must decode to ${KEY_LENGTH} bytes.`);
  return buf;
}
