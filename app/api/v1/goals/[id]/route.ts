...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await logAudit(supabase, u.id, "goal_updated", { id, before, after });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await supabase.from("goals").delete().eq("id", id).eq("user_id", u.id);
    if (error) throw error;

    await logAudit(supabase, u.id, "goal_deleted", { id, deleted: before });
